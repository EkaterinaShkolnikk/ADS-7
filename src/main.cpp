...
int measureOps(int n, int mode, unsigned int* seed) {
    Train train;
    for (int i = 0; i < n; i++) {
        bool state;
        if      (mode == 0) state = false;
        else if (mode == 1) state = true;
        else                state = (rand_r(seed) % 2 == 0);
        train.addCar(state);
    }
    train.getLength();
...
int main() {
...

    {
        ...
        std::cout << "Длина поезда (ожидается 60): "
            << t.getLength() << std::endl;
        std::cout << "Число шагов: "
            << t.getOpCount() << std::endl;
    }
    ...
    for (int n = 2; n <= MAX_N; n += STEP) {
        int ops_off = measureOps(n, 0, &seed);
        int ops_on  = measureOps(n, 1, &seed);

        int64_t sum = 0;
        for (int r = 0; r < REPEATS; r++) sum += measureOps(n, 2, &seed);
